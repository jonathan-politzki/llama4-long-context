...
if [ $# -eq 0 ]; then
    ...
    echo "  compare SIZE      - Run the comparison with specified character count"
    echo "  compare-full SIZE - Run the comparison without test mode"
    echo "  shell             - Get a shell inside the container"
    echo ""
    echo "Examples:"
    echo "  ./run-docker.sh setup"
    echo "  ./run-docker.sh compare 100000"
    echo "  ./run-docker.sh compare-full 1000000"
    echo "  ./run-docker.sh llama"
    exit 0
fi
...
case $COMMAND in
    ...
    compare)
        SIZE=${1:-10000}
        echo "Running comparison with character count $SIZE in test mode..."
        docker-compose run --rm llama-test compare $SIZE --test-mode
        ;;
    compare-full)
        SIZE=${1:-10000}
        echo "Running full comparison with character count $SIZE..."
